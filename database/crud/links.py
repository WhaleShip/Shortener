...
async def add_new_link(original_url, suffix, session: AsyncSession,  owner: User):
    new_link = Link(url=original_url,
                    suffix=suffix,
                    owner_id=owner.id,
                    owner=owner)
...
async def delete_link(link_suffix: str, owner_id: int, session: AsyncSession, ):
    query_result = await session.execute(
        select(Link).filter(Link.suffix == link_suffix, Link.owner_id == owner_id).with_for_update()
    )
