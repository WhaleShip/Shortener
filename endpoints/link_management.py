...
from database.crud import get_links_by_user, add_new_link, delete_link
...
async def get_user_links(session: AsyncSession = Depends(get_session),
                         current_user: User = Depends(get_current_user),):
...
async def delete_link_endpoint(link_suffix: str,
                      session: AsyncSession = Depends(get_session),
                      current_user: User = Depends(get_current_user)):
    deleted = await delete_link(link_suffix, current_user.id, session)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Link not found or not owned by user.")
